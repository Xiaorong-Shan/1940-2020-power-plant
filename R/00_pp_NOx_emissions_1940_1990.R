...
# ===============================================================
# 3. Compute NOx emission factors and additional variables
# ===============================================================
# Build NOx EF (g/kWh)
EF_NOx <- subset_after_1940 %>%
  mutate(
    ...
    scrubber = ifelse(grepl("scrubber|FGD", NOx.Controls, ignore.case = TRUE), 1, 0),
    log_Gen_kWh = log10(Gross.Load..MWh. * 1000 + 1),
    NOx_grams = NOx.Mass..short.tons. * 907185,
    log_NOx_grams = log10(NOx_grams + 1),
    EF = NOx_grams / (Gross.Load..MWh. * 1000),
    log_EF = log10(EF + 1),
...
# ===============================================================
df <- EF_NOx
df$convert_fuel <- factor(df$convert_fuel)
df$Unit.Type    <- factor(df$Unit.Type)
...
# ===============================================================
# 11. Main loop: estimate NOx for each decade (1940–1990)
# ===============================================================
for (yr in target_years) {
  ...
  data_year[, NOx_grams := Predicted_EF_g_per_kWh * annual_gen_kWh]
  data_year[, NOx_tons := NOx_grams / 907185]
  
  keep_cols <- c("Year","Plant_ID","Generator_ID","Plant_Name","Plant_State","County",
                 "Longitude","Latitude","convert_fuel","max_capacity",
                 "annual_gen_kWh","Predicted_EF_g_per_kWh","NOx_tons")
...
OUT_DIR = "/Users/xshan2/Library/CloudStorage/OneDrive-GeorgeMasonUniversity-O365Production/GMU_PhD/01_Research/03_powerplant/data/pp_emissions_1940_1990"
out_csv <- file.path(OUT_DIR, "nox_facility_emissions_1940_1990.csv")
...
combined_facility[, NOx_tons := as.numeric(NOx_tons)]
...
# 13.2 Wide format
by_year_fuel_wide <- dcast(by_year_fuel, Year ~ convert_fuel, value.var = "NOx_tons", fill = 0)
print(by_year_fuel_wide)

# 13.3 Total per Year
by_year_total <- combined_facility[
  , .(NOx_tons_total = sum(NOx_tons, na.rm = TRUE)),
  by = Year
][order(Year)]
print(by_year_total)
