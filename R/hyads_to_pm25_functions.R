...
downloader.fn <- function(filename,
                          destination = file.path('/home/xshan2/HAQ_LAB/xshan2/R_Code/powerplant/pp_hyads/Global_meteorology'),
                          dataset = '20thC_ReanV2c') {  # Only one dataset
  ...
  if(!dir.exists(fileloc)) {
    stop(paste("Directory does not exist:", fileloc))
  }
  
  # Check if the file exists
  if(!file.exists(file_NOAA)) {
    stop(paste("File does not exist in the directory:", file_NOAA))
  }
  
  # Read the file as a raster brick
...
extract_year.fn <- function(raster.in = list.met[[1]],
  ...
  # Since only using 20thC_ReanV2c, no need to check for NARR dataset specifics
  # Remove rotation as it was only for NARR
  
  # Take annual mean if required
  if(avg.period == 'year'){
    out <- stackApply(raster.sub, indices = rep(1, 12), fun = mean)
  } else {
    out <- raster.sub  # If not averaging by year, return the monthly data
  }
...
#======================================================================#
usa.functioner <- function( year.in = 1951:1960,
                            list.met,
                            dataset = '20thC_ReanV2c',
                            avg.period = 'year',
                            return.usa.mask = F,
                            return.usa.sub = T){
...
lm.hyads.ddm.holdout <- function( seed.n = NULL,
  ...
  # calculate evaluation metrics
  metrics.out <- rbind(eval.fn( exp( Y.ho.hat$y.hat.lm.cv_single_poly), y.ho, 'lm.cv_single_poly'))
...
state_exposurer <- function( 
  ...
  # ===== meteorology proxy for pre-1951 years =====
  MET_START_YEAR <- 1951
  met_year <- if (year.m < MET_START_YEAR) MET_START_YEAR else year.m
  if (year.m < MET_START_YEAR) {
    message('[INFO] Using surrogate meteorology ', MET_START_YEAR,
            ' for year ', year.m, ' (meteorology not available before 1951).')
  }
  
  month.N <- formatC( month.n, width = 2, flag = '0')
  name.m <- paste0( 'X', met_year, '.', month.N, '.01')
...
hyads_to_pm25_unit <- function( 
    ...
    name.x = 'hyads',
    mask.use = mask.usa,
    met.dest = '/home/xshan2/HAQ_LAB/xshan2/R_Code/powerplant/pp_hyads/Global_meteorology',
    total = F,
    p4s
){
  ...
  layer.names <- c( "air.2m.mon.mean.nc",
                    "prate.mon.mean.nc", # named "apcp.mon.mean.nc" in NOARR
                    "rhum.2m.mon.mean.nc",
                    "vwnd.10m.mon.mean.nc",
                    "uwnd.10m.mon.mean.nc")
                    
#"rhum.2m.mon.mean.nc": https://www.psl.noaa.gov/thredds/catalog/Datasets/20thC_ReanV2c/Monthlies/pressure/catalog.html

  names( layer.names) <- c( "temp", "apcp", "rhum", "vwnd", "uwnd")
  #names( layer.names) <- c( "temp", "vwnd", "uwnd")
  ...
  # and list.met$temp is the target for matching resolution and extent
  rhum_resampled <- resample(list.met$rhum, list.met$temp, method="bilinear")

  # Replace the original rhum layer with the resampled version
  list.met$rhum <- rhum_resampled
  ...
  if( is.null( month.n)){
    ...
    mets.use.p <- suppressWarnings( 
      usa.functioner( year.m, list.met, dataset = '20thC_ReanV2c', 
    ...
    mets.m <- suppressWarnings( 
      usa.functioner( met_year, list.met, dataset = '20thC_ReanV2c', 
